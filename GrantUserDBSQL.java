...
class CheckBoxRenderer extends JCheckBox implements TableCellRenderer {
	...
	String db_name, user_name;
  CheckBoxRenderer(String db, String user, Connection con) {
  	db_name=db;
  	user_name=user;
  	cons=con;
	 ...
	 if(hasFocus){
	 	...
	 	try{
			stmt=cons.createStatement();
         	if(bol)
         		sql="use "+db_name+"  grant "+command +" on "+rolename +" to "+user_name+"";
         	else sql="use "+db_name+"  revoke "+command +" on "+rolename +" from "+user_name+"";
...
public class GrantUserDBSQL extends JPanel {
  ...
  public GrantUserDBSQL(String db, String user,Connection con){
 //   super("MultiComponent Table");
   ...
   /* contenPane=getContentPane();
    ...
    pn.setLayout(new BorderLayout());
   	JLabel lb=new JLabel("PHÂN QUY\u1EC0N CHO USER");
