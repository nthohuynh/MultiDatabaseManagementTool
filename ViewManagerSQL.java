...
import java.util.*;
import java.sql.*;
public class ViewManagerSQL extends JPanel implements ActionListener
{
//-----------------Khai bao cac bien dung tr2ong chuong trinh--
	...
	ResultSet rss;
//-----------------Ham cau tu-------------------------------
	public ViewManagerSQL(String username,Connection con){

		cons=con;
		stSQL = new String("USE "+username+" EXEC sp_tables @TABLE_TYPE = \"'view'\"");
		us=username;
	...
	//	contenPane.setLayout(new BorderLayout());
		...
		JLabel jlObject =new JLabel("Views t\u1EA1i t\u1EA1i c\u01A1 s\u1EDF d\u1EEF li\u1EC7u: "+us );
		jlObject.setFont(new java.awt.Font("Microsoft Sans Serif", 0, 25));
		jlObject.setIcon(new ImageIcon("icon\\oc4j32.gif"));
		jPanel.add(jlObject);
...
public void deleteView(){
	...
	String SQL="use "+us+" drop view " +(String)(tableResult.getValueAt(row,2)) + "";
...
//-----------Ham su ly su kien click Edit----------
		...
		if (ae.getActionCommand().equals("Xóa"))
		{
			if (tableResult.getSelectedRow()==-1)
			{
				...
				switch (JOptionPane.showConfirmDialog(this,confirm,"Thông báo",1))
					...
					//	connect();
						executeStatement("USE "+us+" EXEC sp_tables @TABLE_TYPE = \"'view'\"");
	...
	//-----------Su ly su kien addUser-----------
		...
		{
				CreateViewSQL gd=new CreateViewSQL("T\u1EA1o m\u1EDBi view",us,cons);
