...
import java.util.*;
import java.sql.*;
public class ViewManagerOracle extends JPanel implements ActionListener
{
	...
	JTable tableResult;
	String username,us;
	public ViewManagerOracle(String username, Connection cons){
		
		con=cons;
		stSQL = new String("SELECT  VIEW_NAME,  TEXT_LENGTH , TEXT   FROM all_views where owner='"+username+"'");
		us=username;
		...
		JLabel jlObject =new JLabel("Views t\u1EA1i schema: "+us );
			...
			if (tableResult.getSelectedRow()==-1)
			{
				...
				switch (JOptionPane.showConfirmDialog(this,confirm,"Thông báo",1))
					...
					case JOptionPane.YES_OPTION:{
						...
						String str="SELECT  VIEW_NAME,  TEXT_LENGTH , TEXT   FROM all_views where owner='"+us+"'";
